...
class Baseline:
    ...
    def run_test_exp(self, env, proxy_reward, policy=None, discount=DISCOUNT, render=False):
    ...
    def agent_learn(self, proxy_reward):

        # note here states = location in the gridworld
        def dyna_q(iter, Q, reward_model, nextS_model, env, proxy_reward, WORLD_WIDTH, scr=None, plan_count=5,
                   n=3000, eplison=0.1, alpha=0.01, discount=0.95):
            ...
            for time_step in range(n):
                ...
                Q[cur_state, chosen_act] += alpha * (
                            reward + discount * np.max(Q[next_state, :]) - Q[cur_state, chosen_act])
                # model update
                ...
                if scr:
                    ...
                    if done:
                        status_update = "exp FINISH at time_step: {} with totalReward: {}".format(time_step,
                                                                                                  totalReward[
                                                                                                      time_step])
                        ...
                        status_update = "exp continues at time_step: {} with totalReward: {}".format(time_step,
                                                                                                     totalReward[
                                                                                                         time_step])
                        scr.addstr(last_row + 1, 0, status_update)
                        scr.refresh()
        ...
        solution_policy_scores, solution_policy_traj_phi, solution_traj = self.run_test_exp(env, proxy_reward, solution_policy, DISCOUNT,False)
        # if training_env:
...
if __name__ == "__main__":
    ...
    policy = baseline_agent.agent_learn(proxy_reward)
